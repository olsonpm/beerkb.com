...
const arrange = arr => {
  ...
  while (arr.length) {
    ...
    if (!arr[outerIdx].length) arr.splice(outerIdx, 1);
    else i += 1;
  }
  return res;
};
...
// opposite of arrange
const distribute = r.curry(
  (n, arr) => {
    return r.addIndex(transform)(
      (res, cur, idx) => res[idx % n].push(cur)
      , r.times(() => [], n)
      , arr
    );
  }
...
const mutableRotate = r.curry(
  (n, arr) => {
    if (n === 0 || arr.length === 0) return arr;

    const [remove, attach] = (n > 0)
      ? ['pop', 'unshift']
      : ['shift', 'push'];
